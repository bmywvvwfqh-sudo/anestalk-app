...
// Trigger Instruction Card Click (術前評估、擺位與誘導、恢復室)
async function triggerInstructionCard(cmd, passedTransObj, cardEl) {
    ...
    // Speak translated text in patient's native target language
...
function speakText(text, langCode) {
    ...
    const targetLang = langCode || 'zh-TW';
...
// Online Cloud Neural TTS Safeguard
function playOnlineTTSFallback(text, langCode) {
    try {
        const cleanLang = (langCode || 'zh-TW').split('-')[0];
        const ttsUrl = `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(text)}&tl=${cleanLang}&client=tw-ob`;
        const audio = new Audio(ttsUrl);
        audio.play().then(() => {
            console.log(`🔊 Cloud TTS fallback playing: [${cleanLang}] "${text.substring(0, 15)}..."`);
        }).catch(err => {
            console.warn('Cloud TTS fallback audio blocked:', err);
        });
    } catch(e) {
        console.error('Online TTS Fallback Exception:', e);
    }
}
...
function speakText(text, langCode) {
    ...
    if (!synth) {
        playOnlineTTSFallback(cleanText, targetLang);
        return;
    }
    ...
    utterance.lang = targetLang;
    utterance.rate = 0.88;
    utterance.pitch = 1.25;  // Feminine pitch enhancement
    utterance.volume = 1.0;

    // Try finding explicit female voice first
    const matchedVoice = getStrictFemaleVoice(targetLang);
    if (matchedVoice) {
    ...
    utterance.onstart = () => {
        hasStarted = true;
        console.log(`🔊 Speech started: [${targetLang}] (${matchedVoice ? matchedVoice.name : 'system-default'}) "${cleanText.substring(0, 20)}..."`);
    };

    utterance.onerror = (e) => {
        console.error('Speech error:', e.error, e);
        if (!hasStarted && e.error !== 'interrupted') {
            // Priority Fallback 1: Local Voice without specific voice object
            try {
                ...
                retry.pitch = 1.25;
                retry.volume = 1.0;
                retry.onstart = () => { hasStarted = true; };
                retry.onerror = () => { playOnlineTTSFallback(cleanText, targetLang); };
                synth.speak(retry);
            } catch(err) {
                // Priority Fallback 2: Cloud TTS
                playOnlineTTSFallback(cleanText, targetLang);
            }
        }
    };

    // Timeout Fallback: If browser silently fails to fire onstart within 350ms (e.g. uninstalled voice)
    speakTimeout = setTimeout(() => {
        if (!hasStarted) {
            console.warn('Speech start timeout, triggering cloud TTS fallback...');
            try {
...
async function translateAnesthesiaTextAsync(text, fromLang, toLang) {
    ...
    // 2. Online Real-Time Neural Translation API (Google Neural Engine for 100% full translation)
    try {
        ...
        if (res.ok) {
            ...
            if (data && data[0] && Array.isArray(data[0])) {
                const fullText = data[0].map(chunk => chunk && chunk[0] ? chunk[0] : '').join('');
